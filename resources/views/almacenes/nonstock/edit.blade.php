...
@endsection
@section('content')
<form id="form-registrar-pedido" action="{{ route('nonstock.update', $nonStockRequest->id) }}" method="POST">
    @csrf
    <div>
        <div class="container-fluid">
                            ...
                            <div class="form-group">
                                <div class="form-line">
                                    <select name="sucursal_id" class="form-control select2" required>
                                        ...
                                        @if ($sucursal)
                                            <option value="{{$sucursal->id}}" {{ $nonStockRequest->sucursal_id == $sucursal->id ? 'selected' : '' }}>
                                                <option value="{{$sucursal->id}}" {{ $nonStockRequest->sucursal_id == $sucursal->id ? 'selected' : '' }}>
                                                {{$sucursal->nombre}}
                                            </option>                                                    
                                        @endif
                                ...
                                <div class="form-line">
                                    <select name="subSucursal_id" id="subSucursal_id" class="form-control select2" required>
                                        ...
                                        @foreach ($subalmacen as $item)
                                            <option value="{{$item->id}}" {{ $nonStockRequest->subSucursal_id == $item->id ? 'selected' : '' }}>
                                                {{$item->name}}
                                            </option>                                                    
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-12">
                                <div class="table-responsive">
                                    <table id="dataTable" class="tables table-bordered table-hover">
                                        ...
                                        <tbody id="table-body">
                                            <tr id="tr-empty" style={{ $nonRequestArticlesExist->isEmpty() ? '' : 'display:none'}}>
                                            ...
                                            @foreach ( $nonRequestArticlesExist as $product )
                                                <tr class="tr-item classRemove" id="tr-item-{{$product->article->id}}">
                                                    <td class="td-item">{{ $loop->iteration}}</td>
                                                    <td>
                                                        <b class="label-description" id="description-{{$product->article->id}}"><small>{{$product->article->nombre}}</small><br>
                                                        <b class="label-description"><small>{{$product->article->presentacion}}</small>
                                                        <input type="hidden" name="article_id[]" value="{{$product->article->id}}" />
                                                    </td>
                                                    <td>
                                                        <input type="number" name="cantidad[]" min="0.1" step="0.01" id="select-cant-{{$product->article->id}}" value="{{$product->quantity}}" style="text-align: right" class="form-control text" required>
                                                    </td>
                                                    <td class="text-right"><button type="button" onclick="removeTr({{$product->article->id}})" class="btn btn-link"><i class="voyager-trash text-danger"></i></button></td>
                                                </tr>
                                            @endforeach   
                            ...
                            <div class="table-responsive">
                                <table id="tblArticles" class="table table-bordered table-striped table-hover">
                                    ...
                                    <tbody id="article-container">
                                        ...
                                        @foreach ( $nonRequestArticlesNoExist as $inexproduct )
                                            <tr data-id="{{ $loop->iteration}}" class="fila">
                                                <td>
                                                    <span class="num-fila">{{ $loop->iteration}}</span>
                                                </td>
                                                <td>
                                                    <input id="article_name" list="articleList" type="text" name="article_name[]" class="article_input form-control" value="{{$inexproduct->nonStockArticle->name_description}}" required>
                                                </td>
                                                <td>
                                                    <input id="presentation" type="text" list="presentationList" name="unit_presentation[]" class="presentation-input form-control" value="{{$inexproduct->articlePresentation->name_presentation}}"required>
                                                </td>
                                                <td>
                                                    <input type="number" name="quantity[]" id="quantity" class="form-control" step="1" value="{{$inexproduct->quantity}}" required>
                                                </td>

                                                <td>
                                                    <button type="button" class="btn btn-danger btn-sm btn-delete-row" data-id="{{ $loop->iteration}}"><i class="voyager-trash"></i></button>
                                                </td>
                                            </tr>
                                        @endforeach
