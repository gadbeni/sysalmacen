...
class EgressController extends Controller
{
        ...
        switch ($type) {
            ...
            case 'solicitud':
                $data =  SolicitudPedido::with(['solicitudDetalle'])
                    ...
                    // ->where('deleted_at', NULL)
                    ->whereRaw('(status = "Aprobado" or status = "Entregado" or status = "eliminado")')
                    ->whereRaw($query_filter)
                    ->orderBy('id', 'DESC')->paginate($paginate);
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $egreso->update([
                'sucursal_id' => $request->branchoffice_id, 'unidadadministrativa' => $request->unidadadministrativa, 'direccionadministrativa' => $request->direccionadministrativa,
